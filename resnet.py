...
def _resnet(arch, block, layers, progress, opt, **kwargs):
    model = ResNet(block, layers, num_classes=opt.num_classes, **kwargs)
...
def load_pretrained_weights(model, pretrained_state_dict):
    ...
    # Print the keys of loaded parameters
    # print('Loaded params keys (from model):', set(model_state_dict.keys()) - set(matched_layers))
